...
def launch(args: list[str], log_name: str) -> None:
    ...
    log = open(logs / log_name, "a", encoding="utf-8")
    subprocess.Popen([sys.executable, str(CONTROLLER), *args], stdin=subprocess.DEVNULL, stdout=log, stderr=log, start_new_session=True, close_fds=True)
    log.close()
...
def main() -> int:
    ...
    record = {"session_id": session_id, "cwd": event.get("cwd"), "model": event.get("model"), "event": name, "updated_at": time.time()}
    ...
    if name == "SessionStart":
        ...
        try:
            ...
            if payload.get("source_session_id") != session_id or pathlib.Path(payload.get("cwd", "")).resolve() != pathlib.Path(event.get("cwd", "")).resolve():
                os.replace(claimed, claimed.with_suffix(".rejected.json"))
                return 0
        except Exception:
            os.replace(claimed, claimed.with_suffix(".rejected.json"))
            return 0
        launch(["run", "--request", str(claimed)], f"{payload['chain_id']}-{payload['generation']}.log")
        print(json.dumps({"continue": True, "systemMessage": "A fresh phase continuation was dispatched by the rollover controller."}))
    return 0
